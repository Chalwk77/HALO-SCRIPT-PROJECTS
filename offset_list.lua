...
function OnScriptLoad(process, Game, persistent)
	...
	halo_broadcast_game = readstring(broadcast_game_address, 5) -- Confirmed. Basically determines whether the server will broadcast on PC/CE/Trial ("halor" = PC, "halom" = CE, "halo?" = Trial).
	...
	if game == "PC" then
		...
		halo_broadcast_game = readstring(0x5E4768, 5) -- Confirmed. Basically determines whether the server will broadcast on PC/CE/Trial ("halor" = PC, "halom" = CE, "halo?" = Trial).
...
function OnClientUpdate(player, m_objectId)
	...
	if thisHash ~= nil then
		...
		koth_team_last_time_in_hill = readdword(koth_globals + player*4 + 0x40) -- Confirmed. gameinfo_current_time - this = time since team was last in the hill. (1 sec = 30 ticks)
		...
		race_player_current_checkpoint = readdword(race_globals + player*4 + 0x44) -- Confirmed. Current checkpoint the player needs to go to. Stored very awkwardly. (0x1 = first checkpoint, 0x3 = second checkpoint, 0x7 = 3rd checkpoint, 0xF = 4th checkpoint, 0x1F = 5th checkpoint etc)
		race_team_score = readdword(race_globals + team*4 + 0x88) -- Confirmed.

		--race checkpoint locations
		for i = 0,race_checkpoint_count do
			race_checkpoint_x_coord = readfloat(race_locs + i*0x20) -- Confirmed.
			race_checkpoint_y_coord = readfloat(race_locs + i*0x20 + 0x4) -- Confirmed.
			race_checkpoint_z_coord = readfloat(race_locs + i*0x20 + 0x8) -- Confirmed.
		end
		...
		player_index = readbyte(m_player + 0x67) -- Confirmed. Player memory id/index (0 - 15) (To clarify: this IS the 'player' argument passed to phasor functions)
		...
		if obj_type == 0 or obj_type == 1 then -- check if object is biped or vehicle
			...
			unit_primaryWeaponLastUse = readdword(m_unit + 0x308) -- From DZS. gameinfo_current_time - this = time since this weapon was last swapped to/picked up. (1 second = 30 ticks)
			unit_secondaryWeaponLastUse = readdword(m_unit + 0x30C) -- From DZS. gameinfo_current_time - this = time since this weapon was last swapped to/picked up. (1 second = 30 ticks)
			unit_tertiaryWeaponLastUse = readdword(m_unit + 0x310) -- From DZS. gameinfo_current_time - this = time since this weapon was last swapped to/picked up. (1 second = 30 ticks)
			unit_quaternaryWeaponLastUse = readdword(m_unit + 0x314) -- From DZS. gameinfo_current_time - this = time since this weapon was last swapped to/picked up. (1 second = 30 ticks)
...
function gettagdata(tag_address)
	...
	--how you need to read from the tagdata to get the mapmod you want to read from/write to.
