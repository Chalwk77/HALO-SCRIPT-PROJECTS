...
function OnNewGame(map)
	...
	local TAG_ID = gettagid("eqip", v[1]) table.insert(EQUIPMENT_TAGS, TAG_ID) end
...
function ApplySpeed(player)
	if player then
		...
		registertimer(SPEED_POWERUP_DURATION*(1000),"ResetSpeed",player) -- 1000 milliseconds (1 second increments)
		privatesay(player, "Speed Boost!", false) -- returning false to disable the server prefix!
...
function ResetSpeed(id,count,player)
	...
	privatesay(player, "Speed Reset!", false) -- Returning false to disable the server prefix!
	local name = getname(player)
...
function OnObjectInteraction(player, objId, MapID)
	for i=(0),#EQUIPMENT_TAGS do
		if MapID == EQUIPMENT_TAGS[i] then
			if MapID == Camouflage_Tab_ID then  -- Camo MapID
			ApplySpeed(player) -- Sets player speed (SPEED_POWERUP)
