...
function OnClientUpdate(player)
	if gamemap == "bloodgulch" then -- Checks to see that the Map matches the script.
		if getplayer(player) then -- Resolves Player
			if inSphere(player, coords) == true then -- Checks if the player is in the Portal Sphere
				if isplayerinvehicle(player) == true then -- Checks whether the player is in the vehicle or not. 
					local m_vehicleId =  getplayervehicleid(player)
					local m_vehicle = getobject(m_vehicleId)
					local name = getname(player)
					if readdword(m_vehicle) == rhog_mapId then -- Checks if the vehicle is a Rocket Hog. 
						if getpassengerplayer(m_vehicle) == player then -- Checks if the player is in the Passengers seat.
							movobjectcoords(m_vehicleId, 81.653, -119.480, 0.428) -- Moves the passenger to these coordinates on the map.
