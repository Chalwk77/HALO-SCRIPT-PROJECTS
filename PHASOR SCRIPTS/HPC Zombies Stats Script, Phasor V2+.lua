...
local ZOMBIE_MESSAGE = "YOU ARE A ZOMBIE - INFECT THE HUMANS!"
local TEAMKILL_MESSAGE = "Please do not betray your Team Mates!"
local ZOMBIEINVISIBLE_MESSAGE = "BEWARE: The zombies are now invisible for 30 seconds!"
local TIMER_TEAM_CHANGE_MESSAGE = "Thank you. The game will now continue"
local BLOCKTEAMCHANGE_MESSAGE = "Sorry. You're not allowed to change teams!"
local INFECTED_MESSAGE = " has been infected! WATCH OUT, YOU COULD BE NEXT!"
local ZOMBIEVEHICLE_MESSAGE = "Zombies are not allowed to use this type of vehicle!"
local HUMANVEHICLE_MESSAGE = "Humans are not allowed to use this type of vehicle!"
local REJOIN_MESSAGE = "Please don't leave and rejoin. You've been put back onto your last team."
local ANNOUNCE_LEVEL_UP = " is now a human!"
local KILLED_BY_GUARDIANS = " was killed by a mysterious force..."
local ZOMBIE_BACKTAP_MESSAGE = "Nice backtap!"
local HUMAN_BACKTAP_MESSAGE = "%s slaughtered you from behind."
--=================================================================================================================================================================--
-- COUNTS --
local ZOMBIE_TEAM = 1 					-- 0 is Red, 1 is Blue.
local HUMAN_TEAM = 1 - ZOMBIE_TEAM
local Invis_Time = 2 					-- 		In seconds how long the zombies should be invisible when they crouch.
local ZOMBIES_INVIS_TIME = 30.00			--		All Zombies Invisible (in seconds)
local Human_Speed = 1.2 				--		Speed when not infected.
local Zombie_Speed = 1.7 				-- 		Zombie Speed.
local lastman_Speed = 1.5 				-- 		Last Man Speed.
------------------------------------------------------------------------------------------------------------------------------------------------------------------
local Reset_Speed = 1					-- 		Resets their speed after receiving speed boost from picking up a 'doublespeed_id' or 'full_spec_id'. 
local Speed_Powerup = 2					-- 		Speed Quality - after picking up 'doublespeed_id' or 'full_spec_id'.
local Speed_Powerup_duration = 20			--		Speed when they pickup 'doublespeed_id' or 'full_spec_id'.
------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
local Minimum_Zombie_Count = 2 				-- 		This caps what the zombie count would be w/ ratio (nil is disable).
...
local AlphaZombie_Clip_Count = 50  			-- 		Number of shots in clip.
...
local Zombie_DamageModifier = 5 			--		Damage midifier for the last man (on top of human damage).
...
-- Booleans --
local AnnounceRank = true				-- Announce Player Rank on Join?
local Infect_On_Fall = true				-- Infect player on fall?
local Infect_On_Suicide = true				-- Infect player on suicide?
local Allow_Speed_Boost = true 				-- Allow / Disallow players to use their FlashLight for SpeedBoost.
...
--==========================================================================================--
-- Prefix Globals --
default_script_prefix = "" -- Can be anything you like. Default Prefix: **SERVER**
...
kill_count = 0
local cur_players = 0
local LAST_MAN_HASH = 0
local New_Game_Timer = 0
local cur_human_count = 0
local cur_zombie_count = 0
local alpha_zombie_count = 0
local player_change_timer = 0
local Current_Map = ""
ZOMBIE_VEHICLES = ""
HUMAN_VEHICLES = ""
HUMAN_VEHICLE_BLOCK = ""
...
First_TapTimeOut = {}
DoubleTap_TimeOut = {}
local KILL_COMMAND_COUNT = {}
local logging = true
local RED_ALLOW  = false
local BLUE_ALLOW = true
...
function OnScriptLoad(processId, game, persistent)
	...
	if persistent == true then -- Script does not support persistence, therefore terminate.
                registertimer(2300,"Terminate",{
                "=============================================================",
                "        ~~~~~~~~~~~~~~~~~~~WARNING~~~~~~~~~~~~~~~~~~",
        ...
        if readstring(gametype_base, 0x2C) == "zombies_stats" then -- Verify game mode is setup correctly, else terminate...
                else
                registertimer(2300,"Terminate",{
                "=============================================================",
                "         ~~~~~~~~~~~~~~~~~~~WARNING~~~~~~~~~~~~~~~~",
                ...
                "         ~~~~~~~~~~~~SCRIPT CANNOT BE USED~~~~~~~~~~",
                "Game Mode must be called 'zombies_stats' and the Game Type must be CTF!",
                "============================================================="})
        end  
...
function LogGameStats(filename, value) -- Stats Logging.
	local file = io.open(filename, "a")
	if Write_To_File == true then
		if file then
			local timestamp = os.date("%I:%M:%S%p  -  %A %d %Y") -- 12 hour clock
-- 			%S 	Second (00-61)
...
function PrintPlayerRank(filename, value) -- Stats Logging.
	local file = io.open(filename, "a")
	if Write_To_File == true then
		if file then
			local timestamp = os.date("%I:%M:%S%p  -  %A %d %Y") -- 12 hour clock
...
function getteamplay()  -- Confirmed. (Off = 0) (On = 1)
...
function AuthorizeServer()
        ...
        if file then
			...
			registertimer(2000,"DelayTerminate",{
			"=============================================================",
						"~~~~~~~~~~~~WARNING~~~~~~~~~~~",
						"~~~~~~NO SERVER KEY FOUND~~~~~",
...
function DelayTerminate(id, count, message)
	if message then
		for v=1,#message do
			hprintf(message[v])
		end
	...
	if stage == 1 then
		...
		for i = 0, 15 do 
			if getplayer(i) then
				...
				if readword(getplayer(i) + 0xAE) < 3 then -- Player Deaths
...
function OnServerChat(player, type, message)
	...
	if player ~= nil then
		...
		elseif Message == "@info" then
			...
			privatesay(player, "\"@stuck\":  Find out how to get unstuck!")
			return false
		...
		elseif Message == "@stats" then
			local PLAYER_KDR = ResolvePlayerKDR(player)
...
--	'SERVER_NAME' will print the name of the server as defined in the .init file, to the joining player.
...
function OnPlayerLeave(player)
	for i = 1,3 do -- For 'Spawn Where Killed' and 'Drop Powerup on Death'.
...
-- Spawn Where Killed Settings (SWK)
	if mode == 4 then -- Killed by a player
...
-- Set Player Respawn: (Human_ReSpawn_Time), (Zombie_ReSpawnTime)
	if victim then
			...
			elseif tonumber(Human_ReSpawn_Time) and team ~= ZOMBIE_TEAM then
...
--	if gametype == "Slayer" and killer and getplayer(killer) then writedword(slayer_globals + 0x40 + killer*4, 0) end
		if last_damage[vhash] then
			...
			elseif last_damage[vhash] == "weapons\\plasma rifle\\charged bolt" then
				...
				medals[hash].count.jackofalltrades = medals[hash].count.jackofalltrades + 1 -- EMP Blast
            ...
            elseif last_damage[vhash] == "weapons\\rocket launcher\\explosion" then
				if m_object then
					if readbyte(m_object + 0x2A0) == 1 then -- obj_crouch
	...
	if game_started == true then
		...
		registertimer(200,"CheckGameState", victim)
			KillStats[gethash(victim)].total.deaths = KillStats[gethash(victim)].total.deaths + 1
--=======================================================================================================================================================--			
			if Announce_Killed_By_Guardians then say("**THE FORCE**  " ..getname(victim) .. KILLED_BY_GUARDIANS) end -- Only used if Infect_On_Guardians is false.
			if Infect_On_Guardians then -- Make Zombie if killed by Guardians.
...
--=======================================================================================================================================================--			
		...
		registertimer(200,"CheckGameState", victim)
			...
			local hash = gethash(killer)
			if readword(getplayer(killer) + 0x98) == 2 then -- If Multi Kill is equal to 2 then
			...
			elseif readword(getplayer(killer) + 0x98) == 3 then -- If Multi Kill is equal to 3 then
				...
				medals[hash].count.multikill = medals[hash].count.multikill + 1
			elseif readword(getplayer(killer) + 0x98) == 4 then -- If Multi Kill is equal to 4 then
				...
				medals[hash].count.multikill = medals[hash].count.multikill + 1
			elseif readword(getplayer(killer) + 0x98) == 5 then -- If Multi Kill is equal to 5 then
				...
				medals[hash].count.multikill = medals[hash].count.multikill + 1
			elseif readword(getplayer(killer) + 0x98) == 6 then -- If Multi Kill is equal to 6 then
				...
				medals[hash].count.multikill = medals[hash].count.multikill + 1
			elseif readword(getplayer(killer) + 0x98) == 7 then -- If Multi Kill is equal to 7 then
				...
				medals[hash].count.multikill = medals[hash].count.multikill + 1
			elseif readword(getplayer(killer) + 0x98) == 8 then -- If Multi Kill is equal to 8 then
				...
				medals[hash].count.multikill = medals[hash].count.multikill + 1
			elseif readword(getplayer(killer) + 0x98) == 9 then -- If Multi Kill is equal to 9 then
				...
				medals[hash].count.multikill = medals[hash].count.multikill + 1
			elseif readword(getplayer(killer) + 0x98) >= 10 then -- If Multi Kill is equal to 10 or more then
			...
			if readword(getplayer(killer) + 0x96) == 5 then -- If Killing Spree is 5 then
			...
			elseif readword(getplayer(killer) + 0x96) == 10 then -- If Killing Spree is 10 then
				SendMessage(killer, "Awarded:    +10 cR - Killing Frenzy")
				changescore(killer, 10, plus)
				sprees[hash].count.killingfrenzy = sprees[hash].count.killingfrenzy + 1
				KillStats[hash].total.credits = KillStats[hash].total.credits + 10
				medals[hash].count.sprees = medals[hash].count.sprees + 1
			elseif readword(getplayer(killer) + 0x96) == 15 then -- If Killing Spree is 15 then
				...
				medals[hash].count.sprees = medals[hash].count.sprees + 1
			elseif readword(getplayer(killer) + 0x96) == 20 then -- If Killing Spree is 20 then
				SendMessage(killer, "Awarded:    +20 cR - Rampage")
				changescore(killer, 20, plus)
				sprees[hash].count.rampage = sprees[hash].count.rampage + 1
				KillStats[hash].total.credits = KillStats[hash].total.credits + 20
				medals[hash].count.sprees = medals[hash].count.sprees + 1
			elseif readword(getplayer(killer) + 0x96) == 25 then -- If Killing Spree is 25 then
				SendMessage(killer, "Awarded:    +25 cR - Untouchable")
				changescore(killer, 25, plus)
				sprees[hash].count.untouchable = sprees[hash].count.untouchable + 1
				KillStats[hash].total.credits = KillStats[hash].total.credits + 25
				medals[hash].count.sprees = medals[hash].count.sprees + 1
			elseif readword(getplayer(killer) + 0x96) == 30 then -- If Killing Spree is 30 then
				...
				medals[hash].count.sprees = medals[hash].count.sprees + 1
			elseif readword(getplayer(killer) + 0x96) == 35 then -- If Killing Spree is 35 then
				...
				medals[hash].count.sprees = medals[hash].count.sprees + 1
--			elseif readword(getplayer(killer) + 0x96) >= 40 and spree%5 == 0 then -- If Killing Spree is 40 or more (Every 5 it will say this after 40) then
			elseif readword(getplayer(killer) + 0x96) >= 40 and sprees%5 == 0 then -- If Killing Spree is 40 or more (Every 5 it will say this after 40) then
			...
			if getplayer(killer) then
				if killer ~= nil then
					...
					if objectid ~= nil then
						...
						local kvehicleid = readdword(m_object + 0x11C) -- Fixed
							...
							if seat_index == 1 or seat_index == 2 or seat_index == 3 or seat_index == 4 then
								for i = 0,15 do 
									if getplayer(i) then
										...
										local loopvehicleid = readdword(m_object + 0x11C) -- Fixed
										local seat_index = readword(m_object + 0x2F0)
...
-- 			Killjoy
			...
			if killer and victim then -- Works
...
--=======================================================================================================================================================--			
		elseif mode == 6 then -- mode 6 == Suicide.
		KillStats[gethash(victim)].total.deaths = KillStats[gethash(victim)].total.deaths + 1
...
--=======================================================================================================================================================--
	if getplayer(player) then -- For 'Spawn Where Killed'.
		if Spawn_Where_Killed == true then
...
function destroyweapons(player)
	for i = 0,3 do -- loop through the player's weapons (primary to quartenary)
		local m_weaponId = getweaponobjectid(player, i)
		if m_weaponId ~= 0xFFFFFFFF then destroyobject(m_weaponId) end
	end
end
				...
				elseif tagName == "powerups\\active camouflage" then
					if isplayerinvis(player) == false then
						if getrandomnumber(1, 11) > 6 then
							...
							if Broadcast_Zombies_Invisible == true then say(ZOMBIEINVISIBLE_MESSAGE)
...
--			elseif isoktoenter("zombies", tagname) == true and AllowThisPlayerInVehicle == true then say("SUCCESS SUCCESS SUCCESS")	
				elseif isoktoenter("zombies", tagname) == true then return true end
				elseif getteam(player) == HUMAN_TEAM then if isoktoenter("humans", tagname) == false then
...
function SetVehicleSettings()
	if map_name == "bloodgulch" then
		if readstring(gametype_base, 0x2C) == "zombies_stats" then
--			ALLOW_PLAYER_IN_VEHICLE_UPON_LEVELUP = "vehicles\\ghost\\ghost_mp"
			ZOMBIE_VEHICLE_ALLOWED = "vehicles\\rwarthog\\rwarthog | vehicles\\warthog\\mp_warthog"
			HUMAN_VEHICLE_BLOCK = "vehicles\\banshee\\banshee_mp | vehicles\\ghost\\ghost_mp | vehicles\\rwarthog\\rwarthog | vehicles\\warthog\\mp_warthog | vehicles\\scorpion\\scorpion_mp | vehicles\\c gun turret\\c gun turret_mp"
			ZOMBIE_VEHICLES = "Rocket Hogs and Chain Gun Hogs!"
		elseif readstring(gametype_base, 0x2C) == "zombies_stats" then
			ZOMBIE_VEHICLE_ALLOWED = "vehicles\\rwarthog\\rwarthog | vehicles\\warthog\\mp_warthog"
			HUMAN_VEHICLE_BLOCK = "vehicles\\banshee\\banshee_mp | vehicles\\ghost\\ghost_mp | vehicles\\rwarthog\\rwarthog | vehicles\\warthog\\mp_warthog | vehicles\\scorpion\\scorpion_mp | vehicles\\c gun turret\\c gun turret_mp"
			ZOMBIE_VEHICLES = "Rocket Hogs and Chain Gun Hogs!"
			HUMAN_VEHICLES = ""
...
function CloseCall(id, count, killer) -- Cleared
...
function PlayerChangeTimer(id, count)
	...
	if count ~= 10 then
		...
		if allow_change == false or zombsize > 0 then
			...
			return false -- Block team Change
		end
...
--									Assign last Man his Ammo (See: LastManAmmoCount at the top of the script.)	
	...
	if cur_last_man ~= nil then
		...
		GiveInvis = registertimer(1000, "ApplyCamoToLastMan", cur_last_man) -- (1000 = milliseconds)
...
function CheckGameState(player) -- Do Not Touch!
...
-- Human count equal to 0, and zombie count greater than 0, then all players are zombies. Result - End the Game.
		if human_count == 0 and Zombie_Count > 0 then -- All players are Zombies
			All_Players_Zombies(player)
-- Human count greater than 1, and zombie count equal to 0, then there are no zombies left...
		elseif human_count > 1 and Zombie_Count == 0 then -- No Zombies Left
			noZombiesLeft()
-- Human count equal to 1, and zombie count greater than 0, then the human is last man.
		elseif human_count == 1 and Zombie_Count > 0 and cur_last_man == nil then -- Last Man Standing
			OnLastMan()
...
function ResolvePlayerKDR(player)
...
function AwardPlayerMedals(player)
	...
	local hash = gethash(player) -- Get the player's hash.
	if hash then
		if done[hash].medal.sprees == "False" then
			if medals[hash].class.sprees == "Iron" and medals[hash].count.sprees >= 5 then -- If the class is iron and the count is more then 5,
				medals[hash].class.sprees = "Bronze" -- Level it up.
				say(getname(player) .. " has earned a medal : Any Spree Iron!") -- Tell them what they earned.
			elseif medals[hash].class.sprees == "Bronze" and medals[hash].count.sprees >= 50 then -- If the class is bronze and the count is more then 50.
				medals[hash].class.sprees = "Silver" -- Level it up.
				say(getname(player) .. " has earned a medal : Any Spree Bronze!") -- Tell them what they earned.
			elseif medals[hash].class.sprees == "Silver" and medals[hash].count.sprees >= 250 then -- If the class is silver and the clount is more then 250.
				medals[hash].class.sprees = "Gold" -- Level it up.
				say(getname(player) .. " has earned a medal : Any Spree Silver!") -- Tell them what they earned.
			elseif medals[hash].class.sprees == "Gold" and medals[hash].count.sprees >= 1000 then -- If the class is gold and the count is more then 1000.
				medals[hash].class.sprees = "Onyx" -- Level it up.
				say(getname(player) .. " has earned a medal : Any Spree Gold!") -- Tell them what they earned.
			...
			elseif medals[hash].class.sprees == "Onyx" and medals[hash].count.sprees >= 4000 then -- If the class is onyx and the count is more then 4000.
				medals[hash].class.sprees = "MAX" -- Level it up.
				say(getname(player) .. " has earned a medal : Any Spree Onyx!") -- Tell them what they earned.
			...
			elseif medals[hash].class.sprees == "MAX" and medals[hash].count.sprees == 10000 then -- if the class is max and the count is 10000.
				say(getname(player) .. " has earned a medal : Any Spree MAX!") -- Tell them what they have earned.
		...
		if done[hash].medal.assists == "False" then
			if medals[hash].class.assists == "Iron" and medals[hash].count.assists >= 50 then -- If the class is iron and the count is more then 50.
				medals[hash].class.assists = "Bronze" -- Level it up.
				say(getname(player) .. " has earned a medal : Assistant Iron!") -- Tell them what they earned.
			elseif medals[hash].class.assists == "Bronze" and medals[hash].count.assists >= 250 then -- If the class is bronze and the count is more then 250.
				medals[hash].class.assists = "Silver" -- Level it up.
				say(getname(player) .. " has earned a medal : Assistant Bronze!") -- Tell them what they earned.
			elseif medals[hash].class.assists == "Silver" and medals[hash].count.assists >= 1000 then -- If the class is silver and the count is more then 1000.
				medals[hash].class.assists = "Gold" -- Level it up.
				say(getname(player) .. " has earned a medal : Assistant Silver!") -- Tell them what they earned.
			elseif medals[hash].class.assists == "Gold" and medals[hash].count.assists >= 4000 then -- If the class is gold and the count is more then 4000.
				medals[hash].class.assists = "Onyx" -- Level it up.
				say(getname(player) .. " has earned a medal : Assistant Gold!") -- Tell them what they earned.
			...
			elseif medals[hash].class.assists == "Onyx" and medals[hash].count.assists >= 8000 then -- If the class is onyx and the count is more then 8000.
				medals[hash].class.assists = "MAX" -- Level it up.
				say(getname(player) .. " has earned a medal : Assistant Onyx!") -- Tell them what they earned.
			...
			elseif medals[hash].class.assists == "MAX" and medals[hash].count.assists == 20000 then -- If the class is max and the count is 20000.
				say(getname(player) .. " has earned a medal : Assistant MAX!") -- Tell them what they earned.
		...
		if done[hash].medal.closequarters == "False" then
			if medals[hash].class.closequarters == "Iron" and medals[hash].count.closequarters >= 50 then -- If the class is iron and the count is more then 50.
				medals[hash].class.closequarters = "Bronze" -- Level it up.
				say(getname(player) .. " has earned a medal : Close Quarters Iron!") -- Tell them what they earned.
			elseif medals[hash].class.closequarters == "Bronze" and medals[hash].count.closequarters >= 125 then -- If the class is bronze and the count is more then 125.
				medals[hash].class.closequarters = "Silver" -- Level it up.
				say(getname(player) .. " has earned a medal : Close Quarters Bronze!") -- Tell them what they earned.
			elseif medals[hash].class.closequarters == "Silver" and medals[hash].count.closequarters >= 400 then -- If the class is silver and the count is more then 400.
				medals[hash].class.closequarters = "Gold" -- Level it up.
				say(getname(player) .. " has earned a medal : Close Quarters Silver!") -- Tell them what they earned.
			elseif medals[hash].class.closequarters == "Gold" and medals[hash].count.closequarters >= 1600 then -- If the class is gold and the count is more then 1600.
				medals[hash].class.closequarters = "Onyx" -- Level it up.
				say(getname(player) .. " has earned a medal : Close Quarters Gold!") -- Tell them what they earned.
			...
			elseif medals[hash].class.closequarters == "Onyx" and medals[hash].count.closequarters >= 4000 then -- If the class is onyx and the count is more then 4000.
				medals[hash].class.closequarters = "MAX" -- Level it up.
				say(getname(player) .. " has earned a medal : Close Quarters Onyx!") -- Tell them what they earned.
				KillStats[hash].total.credits = KillStats[hash].total.credits + 1500
				privatesay(player, "Awarded:    +1500 cR - Close Quarters : Onyx")
				changescore(player, 1500, plus)
			elseif medals[hash].class.closequarters == "MAX" and medals[hash].count.closequarters == 8000 then -- If the class is max and the count is 8000.
				say(getname(player) .. " has earned a medal : Close Quarters MAX!") -- Tell them what they earned.
		...
		if done[hash].medal.crackshot == "False" then
			if medals[hash].class.crackshot == "Iron" and medals[hash].count.crackshot >= 100 then -- If the class is iron and the count is more then 100 .
				medals[hash].class.crackshot = "Bronze" -- Level it up.
				say(getname(player) .. " has earned a medal : Crack Shot Iron!") -- Tell them what they earned.
			elseif medals[hash].class.crackshot == "Bronze" and medals[hash].count.crackshot >= 500 then -- If the class is bronze and the count is more then 500.
				medals[hash].class.crackshot = "Silver" -- Level it up.
				say(getname(player) .. " has earned a medal : Crack Shot Bronze!") -- Tell them what they earned.
			elseif medals[hash].class.crackshot == "Silver" and medals[hash].count.crackshot >= 4000 then -- If the class is silver and the count is more then 4000.
				medals[hash].class.crackshot = "Gold" -- Level it up.
				say(getname(player) .. " has earned a medal : Crack Shot Silver!") -- Tell them what they earned.
			elseif medals[hash].class.crackshot == "Gold" and medals[hash].count.crackshot >= 10000 then -- If the class is gold and the count is more then 10000.
				medals[hash].class.crackshot = "Onyx" -- Level it up.
				say(getname(player) .. " has earned a medal : Crack Shot Gold!") -- Tell them what they earned.
				KillStats[hash].total.credits = KillStats[hash].total.credits + 1000
				privatesay(player, "Awarded:    +1000 cR - Crack Shot : Gold")
				changescore(player, 1000, plus)
			elseif medals[hash].class.crackshot == "Onyx" and medals[hash].count.crackshot >= 20000 then -- If the class is onyx and the count is more then 20000.
				medals[hash].class.crackshot = "MAX" -- Level it up.
				say(getname(player) .. " has earned a medal : Crack Shot Onyx!") -- Tell them what they earned.
				KillStats[hash].total.credits = KillStats[hash].total.credits + 2000
				privatesay(player, "Awarded:    +2000 cR - Crack Shot : Onyx")
				changescore(player, 2000, plus)
			elseif medals[hash].class.crackshot == "MAX" and medals[hash].count.crackshot == 32000 then -- If the class is max and the count is 32000.
				say(getname(player) .. " has earned a medal : Crack Shot MAX!") -- Tell them what they earned.
				KillStats[hash].total.credits = KillStats[hash].total.credits + 3000
		...
		if done[hash].medal.downshift == "False" then
			if medals[hash].class.downshift == "Iron" and medals[hash].count.downshift >= 5 then -- If the class is iron and count is more then 5.
				medals[hash].class.downshift = "Bronze" -- Level it up.
				say(getnamye(player) .. " has earned a medal : Downshift Iron!") -- Tell them what they earned.
			elseif medals[hash].class.downshift == "Bronze" and medals[hash].count.downshift >= 50 then -- If the class is bronze and count is more then 50.
				medals[hash].class.downshift = "Silver" -- Level it up
				say(getname(player) .. " has earned a medal : Downshift Bronze!") -- Tell them what the earned.
			elseif medals[hash].class.downshift == "Silver" and medals[hash].count.downshift >= 750 then -- If the class is silver and count is more then 750.
				medals[hash].class.downshift = "Gold" -- Level it up.
				say(getname(player) .. " has earned a medal : Downshift Silver!") -- Tell them what they earned.
			elseif medals[hash].class.downshift == "Gold" and medals[hash].count.downshift >= 4000 then -- If the class is gold and count is more then 4000.
				medals[hash].class.downshift = "Onyx" -- Level it up.
				say(getname(player) .. " has earned a medal : Downshift Gold!") -- Tell them what they earned.
			...
			elseif medals[hash].class.downshift == "Onyx" and medals[hash].count.downshift >= 8000 then -- If the class is onyx and count is more then 8000.
				medals[hash].class.downshift = "MAX" -- Level it up.
				say(getname(player) .. " has earned a medal : Downshift Onyx!") -- Tell them what they earned.
				KillStats[hash].total.credits = KillStats[hash].total.credits + 1000
				privatesay(player, "Awarded:    +1000 cR - Downshift : Onyx")
				changescore(player, 1000, plus)
			elseif medals[hash].class.downshift == "MAX" and medals[hash].count.downshift == 20000 then -- If the class is max and count is 20000.
				say(getname(player) .. " has earned a medal : Downshift MAX!") -- Tell them what they earned.
				KillStats[hash].total.credits = KillStats[hash].total.credits + 1500
		...
		if done[hash].medal.grenadier == "False" then
			if medals[hash].class.grenadier == "Iron" and medals[hash].count.grenadier >= 25 then -- If the class is iron and count is more then 25.
				medals[hash].class.grenadier = "Bronze" -- Level it up.
				say(getname(player) .. " has earned a medal : Grenadier Iron!") -- Tell them what they earned.
			elseif medals[hash].class.grenadier == "Bronze" and medals[hash].count.grenadier >= 125 then -- If the class is bronze and count is more then 125.
				medals[hash].class.grenadier = "Silver" -- Level it up.
				say(getname(player) .. " has earned a medal : Grenadier Bronze!") -- Tell them what they earned.
			elseif medals[hash].class.grenadier == "Silver" and medals[hash].count.grenadier >= 500 then -- If the class is silver and count is more then 500.
				medals[hash].class.grenadier = "Gold" -- Level it up.
				say(getname(player) .. " has earned a medal : Grenadier Silver!") -- Tell them what they earned.
			elseif medals[hash].class.grenadier == "Gold" and medals[hash].count.grenadier >= 4000 then -- If the class is gold and count is more then 4000.
				medals[hash].class.grenadier = "Onyx" -- Level it up.
				say(getname(player) .. " has earned a medal : Grenadier Gold!") -- Tell them what they earned.
				KillStats[hash].total.credits = KillStats[hash].total.credits + 750
				privatesay(player, "Awarded:    +750 cR - Grenadier : Gold")
				changescore(player, 750, plus)
			elseif medals[hash].class.grenadier == "Onyx" and medals[hash].count.grenadier >= 8000 then -- If the class is onyx and count is more then 8000.
				medals[hash].class.grenadier = "MAX" -- Level it up
				say(getname(player) .. " has earned a medal : Grenadier Onyx!") -- Tell them what they earned.
				KillStats[hash].total.credits = KillStats[hash].total.credits + 1500
				privatesay(player, "Awarded:    +1500 cR - Grenadier : Onyx")
				changescore(player, 1500, plus)
			elseif medals[hash].class.grenadier == "MAX" and medals[hash].count.grenadier == 14000 then -- If the class is max and count is 14000.
				say(getname(player) .. " has earned a medal : Grenadier MAX!") -- Tell them what they earned.
				KillStats[hash].total.credits = KillStats[hash].total.credits + 2250
		...
		if done[hash].medal.jackofalltrades == "False" then
			if medals[hash].class.jackofalltrades == "Iron" and medals[hash].count.jackofalltrades >= 50 then -- Create the jackofalltrades count table.
...
function GetPlayerRank(player)
		...
		local hash = gethash(player) -- Get the hash of the player.
		if hash then
			KillStats[hash].total.credits = KillStats[hash].total.credits or 0
			if KillStats[hash].total.credits > 0 and KillStats[hash].total.credits ~= nil and KillStats[hash].total.rank ~= nil then
				if KillStats[hash].total.credits >= 0 and KillStats[hash].total.credits < 7500 then -- 0 - 7,500
					KillStats[hash].total.rank = "Recruit" -- Decide his rank.
				elseif KillStats[hash].total.credits > 7500 and KillStats[hash].total.credits < 10000 then -- 7,500 - 10,000
					KillStats[hash].total.rank = "Private" -- Decide his rank.
				elseif KillStats[hash].total.credits > 10000 and KillStats[hash].total.credits < 15000 then -- 10,000 - 15,000
					KillStats[hash].total.rank = "Corporal"  -- Decide his rank.
				elseif KillStats[hash].total.credits > 15000 and KillStats[hash].total.credits < 20000 then -- 15,000 - 20,000
					KillStats[hash].total.rank = "Sergeant" -- Decide his rank.
				elseif KillStats[hash].total.credits > 20000 and KillStats[hash].total.credits < 26250 then -- 20,000 - 26,250 
					KillStats[hash].total.rank = "Sergeant Grade 1" -- Decide his rank.
				elseif KillStats[hash].total.credits > 26250 and KillStats[hash].total.credits < 32500 then -- 26,250 - 32,500
					KillStats[hash].total.rank = "Sergeant Grade 2" -- Decide his rank.
				elseif KillStats[hash].total.credits > 32500 and KillStats[hash].total.credits < 45000 then -- 32,500 - 45,000
					KillStats[hash].total.rank = "Warrant Officer" -- Decide his rank.
				elseif KillStats[hash].total.credits > 45000 and KillStats[hash].total.credits < 78000 then -- 45,000 - 78,000
					KillStats[hash].total.rank = "Warrant Officer Grade 1" -- Decide his rank.
				elseif KillStats[hash].total.credits > 78000 and KillStats[hash].total.credits < 111000 then -- 78,000 - 111,000
					KillStats[hash].total.rank = "Warrant Officer Grade 2" -- Decide his rank.
				elseif KillStats[hash].total.credits > 111000 and KillStats[hash].total.credits < 144000 then -- 111,000 - 144,000
					KillStats[hash].total.rank = "Warrant Officer Grade 3" -- Decide his rank.
				elseif KillStats[hash].total.credits > 144000 and KillStats[hash].total.credits < 210000 then -- 144,000 - 210,000
					KillStats[hash].total.rank = "Captain" -- Decide his rank.
				elseif KillStats[hash].total.credits > 210000 and KillStats[hash].total.credits < 233000 then -- 210,000 - 233,000
					KillStats[hash].total.rank = "Captain Grade 1" -- Decide his rank.
				elseif KillStats[hash].total.credits > 233000 and KillStats[hash].total.credits < 256000 then -- 233,000 - 256,000
					KillStats[hash].total.rank = "Captain Grade 2" -- Decide his rank.
				elseif KillStats[hash].total.credits > 256000 and KillStats[hash].total.credits < 279000 then -- 256,000 - 279,000
					KillStats[hash].total.rank = "Captain Grade 3" -- Decide his rank.
				elseif KillStats[hash].total.credits > 279000 and KillStats[hash].total.credits < 325000 then -- 279,000 - 325,000
					KillStats[hash].total.rank = "Major" -- Decide his rank.
				elseif KillStats[hash].total.credits > 325000 and KillStats[hash].total.credits < 350000 then -- 325,000 - 350,000
					KillStats[hash].total.rank = "Major Grade 1" -- Decide his rank.
				elseif KillStats[hash].total.credits > 350000 and KillStats[hash].total.credits < 375000 then -- 350,000 - 375,000
					KillStats[hash].total.rank = "Major Grade 2" -- Decide his rank.
				elseif KillStats[hash].total.credits > 375000 and KillStats[hash].total.credits < 400000 then -- 375,000 - 400,000
					KillStats[hash].total.rank = "Major Grade 3" -- Decide his rank
				elseif KillStats[hash].total.credits > 400000 and KillStats[hash].total.credits < 450000 then -- 400,000 - 450,000
					KillStats[hash].total.rank = "Lt. Colonel" -- Decide his rank.
				elseif KillStats[hash].total.credits > 450000 and KillStats[hash].total.credits < 480000 then -- 450,000 - 480,000
					KillStats[hash].total.rank = "Lt. Colonel Grade 1" -- Decide his rank.
				elseif KillStats[hash].total.credits > 480000 and KillStats[hash].total.credits < 510000 then -- 480,000 - 510,000
					KillStats[hash].total.rank = "Lt. Colonel Grade 2" -- Decide his rank.
				elseif KillStats[hash].total.credits > 510000 and KillStats[hash].total.credits < 540000 then -- 510,000 - 540,000
					KillStats[hash].total.rank = "Lt. Colonel Grade 3" -- Decide his rank.
				elseif KillStats[hash].total.credits > 540000 and KillStats[hash].total.credits < 600000 then -- 540,000 - 600,000
					KillStats[hash].total.rank = "Commander" -- Decide his rank.
				elseif KillStats[hash].total.credits > 600000 and KillStats[hash].total.credits < 650000 then -- 600,000 - 650,000
					KillStats[hash].total.rank = "Commander Grade 1" -- Decide his rank.
				elseif KillStats[hash].total.credits > 650000 and KillStats[hash].total.credits < 700000 then -- 650,000 - 700,000
					KillStats[hash].total.rank = "Commander Grade 2" -- Decide his rank.
				elseif KillStats[hash].total.credits > 700000 and KillStats[hash].total.credits < 750000 then -- 700,000 - 750,000
					KillStats[hash].total.rank = "Commander Grade 3" -- Decide his rank
				elseif KillStats[hash].total.credits > 750000 and KillStats[hash].total.credits < 850000 then -- 750,000 - 850,000
					KillStats[hash].total.rank = "Colonel" -- Decide his rank.
				elseif KillStats[hash].total.credits > 850000 and KillStats[hash].total.credits < 960000 then -- 850,000 - 960,000
					KillStats[hash].total.rank = "Colonel Grade 1" -- Decide his rank.
				elseif KillStats[hash].total.credits > 960000 and KillStats[hash].total.credits < 1070000 then -- 960,000 - 1,070,000
					KillStats[hash].total.rank = "Colonel Grade 2" -- Decide his rank.
				elseif KillStats[hash].total.credits > 1070000 and KillStats[hash].total.credits < 1180000 then -- 1,070,000 - 1,180,000
					KillStats[hash].total.rank = "Colonel Grade 3" -- Decide his rank.
				elseif KillStats[hash].total.credits > 1180000 and KillStats[hash].total.credits < 1400000 then -- 1,180,000 - 1,400,000
					KillStats[hash].total.rank = "Brigadier" -- Decide his rank.
				elseif KillStats[hash].total.credits > 1400000 and KillStats[hash].total.credits < 1520000 then -- 1,400,000 - 1,520,000
					KillStats[hash].total.rank = "Brigadier Grade 1" -- Decide his rank.
				elseif KillStats[hash].total.credits > 1520000 and KillStats[hash].total.credits < 1640000 then -- 1,520,000 - 1,640,000
					KillStats[hash].total.rank = "Brigadier Grade 2" -- Decide his rank
				elseif KillStats[hash].total.credits > 1640000 and KillStats[hash].total.credits < 1760000 then -- 1,640,000 - 1,760,000
					KillStats[hash].total.rank = "Brigadier Grade 3" -- Decide his rank.
				elseif KillStats[hash].total.credits > 1760000 and KillStats[hash].total.credits < 2000000 then -- 1,760,000 - 2,000,000
					KillStats[hash].total.rank = "General" -- Decide his rank.
				elseif KillStats[hash].total.credits > 2000000 and KillStats[hash].total.credits < 2200000 then -- 2,000,000 - 2,200,000
					KillStats[hash].total.rank = "General Grade 1" -- Decide his rank.
				elseif KillStats[hash].total.credits > 2200000 and KillStats[hash].total.credits < 2350000 then -- 2,200,000 - 2,350,000
					KillStats[hash].total.rank = "General Grade 2" -- Decide his rank.
				elseif KillStats[hash].total.credits > 2350000 and KillStats[hash].total.credits < 2500000 then -- 2,350,000 - 2,500,000
					KillStats[hash].total.rank = "General Grade 3" -- Decide his rank.
				elseif KillStats[hash].total.credits > 2500000 and KillStats[hash].total.credits < 2650000 then -- 2,500,000 - 2,650,000
					KillStats[hash].total.rank = "General Grade 4" -- Decide his rank.
				elseif KillStats[hash].total.credits > 2650000 and KillStats[hash].total.credits < 3000000 then -- 2,650,000 - 3,000,000
					KillStats[hash].total.rank = "Field Marshall"  -- Decide his rank.
				elseif KillStats[hash].total.credits > 3000000 and KillStats[hash].total.credits < 3700000 then -- 3,000,000 - 3,700,000
					KillStats[hash].total.rank = "Hero" -- Decide his rank.
				elseif KillStats[hash].total.credits > 3700000 and KillStats[hash].total.credits < 4600000 then -- 3,700,000 - 4,600,000
					KillStats[hash].total.rank = "Legend" -- Decide his rank.
				elseif KillStats[hash].total.credits > 4600000 and KillStats[hash].total.credits < 5650000 then -- 4,600,000 - 5,650,000
					KillStats[hash].total.rank = "Mythic" -- Decide his rank.
				elseif KillStats[hash].total.credits > 5650000 and KillStats[hash].total.credits < 7000000 then -- 5,650,000 - 7,000,000
					KillStats[hash].total.rank = "Noble" -- Decide his rank.
				elseif KillStats[hash].total.credits > 7000000 and KillStats[hash].total.credits < 8500000 then -- 7,000,000 - 8,500,000
					KillStats[hash].total.rank = "Eclipse" -- Decide his rank.
				elseif KillStats[hash].total.credits > 8500000 and KillStats[hash].total.credits < 11000000 then -- 8,500,000 - 11,000,000
					KillStats[hash].total.rank = "Nova" -- Decide his rank.
				elseif KillStats[hash].total.credits > 11000000 and KillStats[hash].total.credits < 13000000 then -- 11,000,000 - 13,000,000
					KillStats[hash].total.rank = "Forerunner" -- Decide his rank.
				elseif KillStats[hash].total.credits > 13000000 and KillStats[hash].total.credits < 16500000 then -- 13,000,000 - 16,500,000
					KillStats[hash].total.rank = "Reclaimer" -- Decide his rank.
				elseif KillStats[hash].total.credits > 16500000 and KillStats[hash].total.credits < 20000000 then -- 16,500,000 - 20,000,000
					KillStats[hash].total.rank = "Inheritor" -- Decide his rank.
				end
			end
		end
end
...
--	I will add my change logs later. later.