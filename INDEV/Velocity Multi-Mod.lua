...
function RecordChanges()
    ...
    local cl = {}
    cl[#cl + 1] = "[2/22/19]"
    cl[#cl + 1] = "Began Development of BGS (now known as Velocity)"
    cl[#cl + 1] = ""
    cl[#cl + 1] = "-------------------------------------------------------------------------------------------------------------------------------"
    cl[#cl + 1] = "[2/23/19]"
    cl[#cl + 1] = "Core functionality complete"
    cl[#cl + 1] = ""
    cl[#cl + 1] = ""
    cl[#cl + 1] = "-------------------------------------------------------------------------------------------------------------------------------"
    cl[#cl + 1] = "[2/24/19]"
    cl[#cl + 1] = "First semi-stable Release (github: INDEV) - script released on v1.0"
    cl[#cl + 1] = ""
    cl[#cl + 1] = ""
    cl[#cl + 1] = "-------------------------------------------------------------------------------------------------------------------------------"
    cl[#cl + 1] = "[2/25/19]"
    ...
    cl[#cl + 1] = "Additionally, you can toggle on|off garbage collection (on death, on disconnect)"
	cl[#cl + 1] = "in the config sections of the respective mods - script updated to v1.2"
    cl[#cl + 1] = "-------------------------------------------------------------------------------------------------------------------------------"
    cl[#cl + 1] = ""
    cl[#cl + 1] = ""
    cl[#cl + 1] = "[2/26/19]"
    cl[#cl + 1] = "You can now use Lurker mode while Infinity Ammo is enabled,"
    cl[#cl + 1] = "but you cannot manipulate damage multipliers - script updated to v1.3"
    cl[#cl + 1] = "-------------------------------------------------------------------------------------------------------------------------------"
    cl[#cl + 1] = ""
    cl[#cl + 1] = ""
    cl[#cl + 1] = "[2/27/19]"
    cl[#cl + 1] = "Fixed a map check error relating to Respawn Time - script updated to v1.4"
    cl[#cl + 1] = "-------------------------------------------------------------------------------------------------------------------------------"
    cl[#cl + 1] = ""
    cl[#cl + 1] = ""
    cl[#cl + 1] = "[2/28/19]"
    cl[#cl + 1] = "Fixed a bug relating to Anti Impersonator."
    cl[#cl + 1] = "The user table in the Anti Impersonator configuration table now supports multiple hashes per user - script updated to v1.5"
    cl[#cl + 1] = "-------------------------------------------------------------------------------------------------------------------------------"
    cl[#cl + 1] = ""
    cl[#cl + 1] = ""
    cl[#cl + 1] = "[4/1/19]"
    ...
    cl[#cl + 1] = "Bug fix for Alias System and other minor bug fixes - script updated to v1.6"
    cl[#cl + 1] = "-------------------------------------------------------------------------------------------------------------------------------"
    cl[#cl + 1] = ""
    cl[#cl + 1] = ""
    cl[#cl + 1] = "[4/2/19]"
    cl[#cl + 1] = "Completely Rewrote BGS and released under a new name: (Velocity - Multi Mod) - script updated to v1.7"
    cl[#cl + 1] = "-------------------------------------------------------------------------------------------------------------------------------"
    cl[#cl + 1] = ""
    cl[#cl + 1] = ""
    cl[#cl + 1] = "[4/22/19]"
    cl[#cl + 1] = "Updated Anti-Impersonator with optional IP Address scans"
    ...
    cl[#cl + 1] = ""
    cl[#cl + 1] = "[4/25/19]"
    cl[#cl + 1] = "Fixed a bug relating to Alias System"
    cl[#cl + 1] = "A couple of documentation edits"
	cl[#cl + 1] = ""
	cl[#cl + 1] = "Script Updated to v1.0"
    cl[#cl + 1] = "-------------------------------------------------------------------------------------------------------------------------------"
    cl[#cl + 1] = ""
    cl[#cl + 1] = ""
    cl[#cl + 1] = "[4/27/19]"
    cl[#cl + 1] = "Bug Fix relating to function 'OnPlayerDisconnect()'"
    cl[#cl + 1] = "Bug Fix relating to function 'velocity:loadMute()'"
    cl[#cl + 1] = "Bug Fix for Suggestion Box"
    cl[#cl + 1] = "Minor Bug Fixes"
    cl[#cl + 1] = "Began writing Private Messaging System"
    cl[#cl + 1] = "Continued development on Private Messaging System"
	cl[#cl + 1] = ""
