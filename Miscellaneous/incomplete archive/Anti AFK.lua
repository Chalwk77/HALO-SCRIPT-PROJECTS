...
function AntiAFK:OnTick()
    ...
    for i, v in pairs(self.players) do
        ...
        if (v.timer >= self.max_afk_time) then
            execute_command('k ' .. i .. ' "' .. self.kick_reason .. '"')
        end
    end
end

function AntiAFK:OnPreSpawn(Ply)
    self.players[Ply].update_aim = false
end

function AntiAFK:OnSpawn(Ply, DelayUpdate)
    Ply = tonumber(Ply)
    if (not DelayUpdate) then
        timer(500, "OnSpawn", Ply, 1)
    else
        self.players[Ply].update_aim = true
    end
end

function AntiAFK:InitPlayer(Ply, Reset)
    if (not Reset) then
        self.players[Ply] = {
            timer = 0,
...
function OnJoin(Ply)
    AntiAFK:InitPlayer(Ply)
end

function OnQuit(Ply)
    AntiAFK:InitPlayer(Ply, true)
end
...
function OnSpawn(P, U)
    AntiAFK:OnSpawn(P, U)
end

function OnPreSpawn(P)
    AntiAFK:OnPreSpawn(P)
end

function AimTick()
    return AntiAFK:AimTick()
end
