...
function OnClientUpdate(player, m_objectId)
    ...
    if thisHash ~= nil then
        ...
        client_network_struct = network_struct + 0x1AA + ce + player
        client_struct_size -- Strange. It starts in the middle of the dword.
        ...
        client_machineinfo_struct = network_struct + 0x3B8 + ce + player
        client_machineinfo_size
...
function getplayermachinenum(player_number)
    player_number = tonumber(player_number) or raiseerror(bad
    argument
    #1
    to
    getplayermachinenum(valid
    player
    required, got .. tostring(type(player_number)) .. ))
...
function getplayer(player_number)
    ...
    local player_struct = player_base + (player_number  player_struct_size)
    -- Confirmed with getplayer(player).
...
function getobject(m_objectId)
    ...
    local obj_table_flags = rshift(m_objectId, 44) - rshift(m_objectId, 64)
    0x100 -- part of the objId salt
    ...
    local obj_table_address = obj_table_base + (obj_table_index
    obj_table_size) + 0x8 -- Confirmed.
...
function gettagaddress(tagtype, tagname)
    ...
    for i = 0, (tag_table_count - 1) do
        ...
        local tag_class = readstring(tag_table_base, (tag_table_size
        i), 4)
