...
function OnNewGame()
    if CheckType() == true then
        ...
        game_over = false
        if tonumber(death_penalty) > 1 then
            character1 = "s"
        elseif tonumber(death_penalty) == 1 then
            character1 = ""
        end
        if tonumber(suicide_penalty) > 1 then
            character2 = "s"
        elseif tonumber(suicide_penalty) == 1 then
            character2 = ""
        end
    end
end
...
function OnPlayerLeave(PlayerIndex)
    ...
    for k, _ in pairs(trophy_drone_table[drone_player_id]) do
...
function OnTick()
    if (destroy_timer[drone_player_id] == true) then
        if trophy_count[victim_drone] == 0 then
    ...
    for j = 1, 16 do
        if player_present(j) then
            if (info_timer[j] == true) then
                ...
                ClearConsole(j)
                for _, v in pairs(info_board) do
                    rprint(j, v)
                end
